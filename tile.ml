...
(** The type [tile] represents a hex on the map.
    ...
    [ (0,
   \[(9,13);(8,13);(4,8);(1,4)\]); (1, \[(1,5)\]); (2, \[(5,9)\]) ] *)

(* type tile = { resource_name : Resource.t; tile_dice_num : int;
   tile_position : int; neighbor_tiles : tile list; corners : (int *
   building_opt) list; robber : bool; roads : (int * edge list) list; } *)

type t = Adj_matrix.tile

let resource hex = hex.resource

let dice_num hex = hex.dice_num

let position hex = hex.id

let has_robber (hex : Adj_matrix.tile) = hex.robber
