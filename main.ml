...
(* let rec replace_players new_players old_player_list = match
   ...
   match List.filter (fun new_p -> h.num = new_p.num) old_player_list
   with | [ x ] -> x (*should return player one*) | _ -> failwith "2
   players with the same number somehow " in let new_pl_list = (*[1', 2,
   3, 4]*) new_pl :: List.filter (fun new_p -> h.num <> new_p.num)
   old_player_list in replace_players t new_pl_list *)
